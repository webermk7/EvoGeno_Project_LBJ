...
Below, we will begin our analysis into the NEON chemistry characteristics of the Actinomycetota MAGs at the sites, to get a better understanding of the conditions that the bacteria live and thrive in, as well as to find some intersting patterns in the data.
...
To get more information from this tree, we can also color the nodes by Ecosystem subtype, emphasizing the abundacne of Actinomycetota in grasslands, shrublands, and forests. However, there are some examples of Tundra, Desert, and Taiga MAGs as well.
...
## National Grasslands LBJ, and Actinomycetota

Now, lets briefly take a break from analyzing Actinomycetota at all sites, and zoom in on National Grasslands LBJ, or CLBJ. We. will begin, as previously, by assessing the phylogenetic tree.
...
theme(axis.text.x = element_text(size = 15, angle = 0, hjust = 1, vjust = 1),
  ...
  coord_flip() 
```
We can see from the tree and bargraph above that, although a very diverse site, Actinomycetota is the most abundant phylum of bacteria found at CLBJ.
...
ggtree(tree_bac_CLBJ_MAGs_Actino, layout="circular")  %<+%
  ...
  geom_point(mapping=aes(color=Class)) 
```
We can see that, from the tree, Thermoleophilia and Actinomycetia are the two most abundant classes of Actinomycetota at CLBJ. Interesting to note is that although there are Rubrobacteria examples in the entire NEON data, none of them are found at CLBJ. However, we still see the elusive Class UBA4738, of which there is very little published information. We can now go further down the phylogenetic tree to assess the Order of the Actinomycetota bacteria found at CLBJ: 
...
```
In the above tree, we can see that there is only a single phylogenetic group of the order Actinomycetales found at CLBJ (shown in brown in the bottom of the tree). This bacterial order is particularly interesting and known for its ability to grow in long branches, similar in look to fungal growth (DOI: 10.3389/fmicb.2018.00477).




## Thermoleophilia, an Interesting Class of Actinomycetota
...
ggtree(tree_bac_CLBJ_MAGs_Actino, layout="circular")  %<+%
  ...
  geom_point(mapping=aes(color=Order)) 
```
At CLBJ, we only see the two orders Gaiellales and Solirubrobacterales. Now lets assess the families: 
...
```
Another surprising scarcity is discovered; as there is only one phylogenetic branch dedicated to the family Thermoleophilaceae at CLBJ. Furthermore, there is another elusively named phylogeny of the 70-9 family, of which there is very little, if any, published information on. Now, lets zoom in on the Genus of the above examples: 
...
```
From this tree we can see that the single branch of the family Thermoleophilaceae is of the genus Conexibacter.

## NEON Chemistry Analysis of Actinomycetota 
...
```
Above, we clearly see the preferential finding of UBA4738 in more basic environments, with no examples being found below pH 5, compared to four of the other classes being found in high numbers below this pH value.
